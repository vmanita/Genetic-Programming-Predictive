...
class _Program(object):
    ...
    def crossover(self, donor, random_state):
        ...
        #print("ISTO É UM CROSSOVER NORMAL")
    ...
    def simple_crossover(self, donor, random_state):

        #print("ISTO É UM SIMPLE CROSSOVER")
        ...
        while True:
            donor_start, donor_end = self.get_subtree(random_state, donor)
            if not(isinstance(donor[donor_end-1], _Function)):
                subtree = donor[donor_start:donor_end]
                break

        donor_removed = list(set(range(len(donor))) -
                             set(range(donor_start, donor_end)))

        return program[:start] + subtree + program[end:], removed, donor_removed

    def p2_crossover(self, donor1, donor2, random_state):
        pass
        '''
        #print("ISTO É UM P2 CROSSOVER")

        program = copy(self.program)

        while True:
            start1, end1 = self.get_subtree(random_state)
            start2, end2 = self.get_subtree(random_state)
            x = range(start1, end1)
            y = range(start2, end2)
            xs = set(x)
            if xs.intersection(y) == set():
                break
        ...
        removed1 = range(start1, end1)
        removed2 = range(start2, end2)
        removed = list(set(range(len(program))) - set(removed1) - set(removed2))

        parent1 = copy(donor1)
        parent2 = copy(donor2)

        while True:
            donor_start1, donor_end1 = self.get_subtree(random_state, parent1)
            donor_start2, donor_end2 = self.get_subtree(random_state, parent2)
            subtree1 = parent1[donor_start1:donor_end1]
            subtree2 = parent2[donor_start2:donor_end2]
            if len(subtree1)>1 and len(subtree2)>1:
                break

        #print("subtree1")
        for i in range(len(subtree1)):
            if isinstance(subtree1[i], _Function):
                print(subtree1[i].name)
            else:
                print(subtree1[i])

        #print("subtree2")
        for i in range(len(subtree2)):
            if isinstance(subtree2[i], _Function):
                print(subtree2[i].name)
            else:
                print(subtree2[i])
        ...
        child = program[:l_start] + subtree1 + program[:l_end] + program[l_end:h_start] + subtree2 + program[h_end:]

        print("child")
        for i in range(len(child)):
            if isinstance(child[i], _Function):
                print(child[i].name)
            else:
                print(child[i])

        donor_removed = list(set(range(len(parent1))) - set(range(len(parent2))) -
                             set(range(donor_start1, donor_end1)) - set(range(donor_start2, donor_end2)))

        return child, removed, donor_removed
        '''
    def uniform_crossover(self, donor, random_state):

        #print("ISTO É UM UNIFORM CROSSOVER")

        child1 = copy(self.program)
        child2 = copy(donor)
        ...
        for node in range(len(size)):
            if isinstance(child1[node], _Function):
                if isinstance(child2[node], _Function):
                    if child1[node].arity == child2[node].arity:
                        common_zones.append(node)
            else:
                if isinstance(child2[node], _Function):
                    continue
                else:
        ...
        for zone in common_zones:
            if random_state.uniform() > 0.5:
                child1[zone], child2[zone] = child1[zone], child2[zone]
        return child1, common_zones
    ...
    def gs_mutation_tanh_semantics(self, X, ms, random_state):
        ...
        #print("gs_mutation_tanh_semantics")
    ...
    def gs_mutation_sig_semantics(self, X, ms, random_state):

        # print("gs_mutation_sig_semantics")

        rt1 = [_function_map['mul'], ms, sig1] + self.build_program(random_state)
        rt2 = [_function_map['mul'], ms, sig1] + self.build_program(random_state)
        offspring_length = self.program_length + np.maximum(len(rt2), len(rt1)) + 1
        offspring_depth = 1 + np.maximum(self.program_depth, np.maximum(self._depth_program(rt1),
                                                                        self._depth_program(rt2)))
        return (np.add(self.program, np.maximum(_Program.execute_(rt1, X), _Program.execute_(rt2, X))),
                offspring_length, offspring_depth)

    def subtree_mutation(self, random_state):

        #print("ISTO É UMA SUBTREE MUTATION")
    ...
    def hoist_mutation(self, random_state):
       ...
       # print("ISTO É UMA HOIST MUTATION")
    ...
    def point_mutation(self, random_state):
        ...
        #print("ISTO É UMA POINT MUTATION")
    ...
    def shake_mutation(self, random_state):

        #print("ISTO É UMA SHAKE MUTATION")

        start, end = self.get_subtree(random_state)
        subtree = self.program[start:end]
        mutate = [i for i in range(len(subtree))]

        for node in mutate:
            if isinstance(subtree[node], _Function):
                arity = subtree[node].arity
                # Find a valid replacement with same arity
                replacement = len(self.arities[arity])
                replacement = random_state.randint(replacement)
                replacement = self.arities[arity][replacement]
                subtree[node] = replacement
            else:
                # We've got a terminal, add a const or variable
                if self.const_range is not None:
                    terminal = random_state.randint(self.n_features + 1)
                ...
                if terminal == self.n_features:
                    ...
                    if self.const_range is None:
                        ...
                        raise ValueError('A constant was produced with '
                                         'const_range=None.')
                subtree[node] = terminal

        return self.program[:start] + subtree + self.program[end:], list(mutate)


    def graft_mutation(self, random_state):

        #print("ISTO É UMA GRAFT MUTATION")

        program = copy(self.program)
        mutate = [i for i in range(len(program)) if not(isinstance(program[i], _Function))]
        node = random_state.choice(mutate)
        while True:
            sub_start, sub_end = self.get_subtree(random_state)
            if isinstance(program[sub_start], _Function):
                subtree = program[sub_start:sub_end]
                break
        program = program[:node] + subtree + program[node+1:]
        return program, list(mutate)


    def reverse_mutation(self, random_state):

        #print("ISTO É UMA REVERSE MUTATION")

        program = copy(self.program)
        mutate = [i for i in range(len(program)) if isinstance(program[i], _Function)]
        reverse_function = {'add': _function_map['sub'],
        ...
        for node in mutate:
            for name, function in reverse_function.items():
                if name == program[node].name:
                    program[node] = function
        return program, list(mutate)

    def swap_mutation(self, random_state):

        #print("ISTO É UMA SWAP MUTATION")
        ...
        if len(mutate)<=1:
            return program, mutate

        counter = 0
        check_impossible = 0

        while True:
            node1 = random_state.choice(mutate)
            node2 = random_state.choice(mutate)
            check_impossible+=1
            if check_impossible == 50:
                break
            if node1 != node2 and program[node1].arity == program[node2].arity:
                program[node1], program[node2] = program[node2], program[node1]
                counter+=1
            if counter == len(mutate):
                break
